...
class XMLPipeline:
    ...
    def check_data(self):
        ...
        self.compare_directory(
            self.image_dir, remove_img_dir, img_name, xml_name, img_ext
        )
        self.compare_directory(
            self.xml_dir, remove_xml_dir, xml_name, img_name, xml_ext
        )
    ...
    def rename_files(self, name, folder_path, number):
        ...
        if natsorted is None:
            raise ImportError(
                "natsorted is required" "Install it using `pip install natsort`"
            )
        ...
        for i, filename in enumerate(natsorted(os.listdir(folder_path)), number):
            ...
            # TODO: notedown the possible exception and update the exception likewise
                ...
                raise Exception(
                    "Failed to rename the files in the given folder please check the folder structure or os permission"
                )
    ...
    def rename_img_xml(self, name, number):
        ...
        self.check_data()
        for img, xml in zip(
            natsorted(os.listdir(self.image_dir)), natsorted(os.listdir(self.xml_dir))
        ):
            try:
    ...
    def create_training_data(self):
        ...
        print(
            "Data will be divided in ratio=====",
            {
                "train": random_no_list[0],
                "test": random_no_list[1],
                "qa": random_no_list[2],
                "val": random_no_list[3],
            },
        )
        ...
        for i, x in enumerate(zip(img_folder, xml_folder)):
            dst_dir = (
                os.path.split(self.image_dir)[0] + "/" + "training_data" + "/" + x[0]
            )
            dst_xml = (
                os.path.split(self.xml_dir)[0] + "/" + "training_data" + "/" + x[1]
            )
            ...
            random_images = np.random.choice(
                file_list, int(random_no_list[i]), replace=False
            )
            for file in list(random_images):
                if not os.path.exists(os.path.join(dst_dir, file)):
                    shutil.copy2(
                        os.path.join(self.image_dir, file), os.path.join(dst_dir, file)
                    )
            ...
            list_dir1 = [
                os.path.splitext(filename)[0] for filename in os.listdir(self.xml_dir)
            ]
            # list of files without extension from image dir
            list_dir2 = [
                os.path.splitext(filename)[0] for filename in os.listdir(dst_dir)
            ]
                ...
                for file in list_dir1:
                    if file in list_dir2:
                        ...
                        if not os.path.exists(os.path.join(dst_xml, move_file)):
                            shutil.copy2(
                                os.path.join(self.xml_dir, move_file),
                                os.path.join(dst_xml, move_file),
                            )
                        else:
                            pass
                    else:
