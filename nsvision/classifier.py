...
def split_image_data(data_dir, ratio, data_path=True, generate_labels_txt=False):
    ...
    if not isinstance(ratio, tuple):
        raise TypeError(
            f"ratio should be tuple of (train,val,test,qa) but got {ratio} of type {type(ratio)} instead"
        )

    if len(ratio) != 4:
        raise ValueError(
            f"required ratio to be of length 4 but got length {len(ratio)} instead"
        )
    # ratio
    ...
    try:
        for class_name in classes_list:
            ...
            print(
                f"\nData divided for class {class_name} as follows:-",
                f"Train: {div1}",
                f"Validation: {div2 - div1}",
                f"Test: {div3 -div2}",
                f"QA: {total_class_images - div3}\n",
                sep="\n",
            )
            ...
            for foldr, lst in zip(folder_names, split_lst):
                ...
                for img in lst:
                    ...
                    dst_path = os.path.join(
                        os.path.join(model_data_dir, foldr), class_name
                    )
        ...
        if data_path:
            ...
            with open(
                os.path.join(model_data_dir, "data_path_history.json"), "w"
            ) as outfile:
        ...
        raise OSError(
            "Failed to split data please check folder structure or check your OS Permission"
        )
    print(
        "Splitting completed", f"Splitted data is stored at {model_data_dir}", sep="\n"
    )
...
#         raise ImportError("h5py is required" "Install using `pip install h5py`")
