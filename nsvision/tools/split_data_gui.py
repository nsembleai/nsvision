...
class SplitDataGui(Tk):
    def __init__(self):
        ...
        self.selectorLabelFrame = ttk.LabelFrame(
            self, text="Selector label frame"
        ).grid(column=0, row=0)

        self.sourceLabelFrame = ttk.LabelFrame(
            self, text="Select Source Folder and set ratio"
        )
        ...
        self.source_button = ttk.Button(
            self.sourceLabelFrame, text="Browse", command=self.source_browse
        )
        self.source_button.grid(column=0, row=0, padx=10, pady=10)

        self.set_ratio_button = ttk.Button(
            self.sourceLabelFrame, text="Set Ratio", command=self.set_ratio
        ...
        self.qa_input_box = Entry(
            self.ratioFrame,
            state="normal",
            textvariable=self.qavar,
            validate="all",
            validatecommand=(vcmd, "%P"),
        )
    ...
    def source_browse(self):
        self.source_browse_path = Path(
            filedialog.askdirectory()
        )  # using Path() to get absolute path for linux as well as windows
    ...
    def set_ratio(self):
        ...
        if total != 100:
            messagebox.showwarning(
                "Please check your ratio",
                f"Total ratio must be 100 but your total ratio is {total}",
            )
        else:
    ...
    def split_data(self):
        try:
            split_image_data(
                self.source_browse_path,
                ratio=(self.train, self.val, self.test, self.qa),
            )
    ...
    def process(self):
        if self.source_browse_path is None or self.train_input_box["state"] == "normal":
            messagebox.showwarning(
                "Warning",
                "Please set input path and ratio before pressing 'Split Data'",
            )
        else:
